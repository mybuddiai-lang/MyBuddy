...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-zinc-100 safe-area-bottom">
          ...
          return (
            ...
            >
              <Icon
                ...
                className={clsx('transition', isActive && 'drop-shadow-sm')}
              />
              <span className={clsx('text-[10px] font-medium truncate', isActive ? 'text-brand-600' : 'text-zinc-400')}>
                {label}
              </span>
              {isActive && (
                <span className="w-1 h-1 rounded-full bg-brand-500 absolute bottom-2" />
              )}
            </Link>
          );
        })}
